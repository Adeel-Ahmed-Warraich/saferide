...
import { CreditCard, UploadCloud, User } from 'lucide-react';
...
const PaymentPage = () => {
  ...
  // All children for this parent account
  const [children, setChildren] = useState([]);
  const [selectedChild, setSelectedChild] = useState(''); // child name label

  useEffect(() => {
    const init = async () => {
      try {
        // Primary child from parent record
        const primary = { label: currentUser.childName, isPrimary: true };

        // Additional approved siblings from enrollments
        ...
        setChildren(all);
        setSelectedChild(primary.label); // default to first child
  ...
  const submitPayment = async (method, extraData = {}) => {
    ...
    await pb.collection('payments').create(base, { $autoCancel: false });
  };
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100 space-y-6">
            ...
            <div>
              ...
              <Input id="amount" type="number" value={amount} onChange={e => setAmount(e.target.value)} className="text-2xl font-bold mt-2 h-14" />
            </div>
