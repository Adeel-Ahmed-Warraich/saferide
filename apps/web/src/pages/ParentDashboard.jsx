...
import { MapPin, CreditCard, Bell, Clock, User, AlertCircle, Info, Briefcase, PhoneCall } from 'lucide-react';
...
const ParentDashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // Fetch latest payment separately so a failure doesn't break the whole dashboard
      try {
        ...
        if (payments.items.length > 0) {
          setLatestPayment(payments.items[0]);
        }
      } catch (error) {
        console.error("Error fetching payments:", error?.status, error?.message);
        // Dashboard still shows without payment info
      }
        ...
        <div className="max-w-5xl mx-auto space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 md:col-span-2">
              <h2 className="text-xl font-bold text-[#1E40AF] mb-4 flex items-center gap-2"><User className="w-5 h-5 text-[#FBBF24]"/> Child Details</h2>
            ...
            <div className="bg-[#1E40AF] text-white p-6 rounded-2xl shadow-lg flex flex-col justify-between relative overflow-hidden">
              ...
              <Link to="/payments" className="relative z-10">
                <Button className="w-full bg-[#FBBF24] hover:bg-yellow-500 text-[#1E40AF] font-bold">
                  Pay Now
                </Button>
              </Link>
            </div>
